...
class Rating: public std::tuple<uint32_t, uint32_t, uint8_t> {
public:
  typedef std::tuple<uint32_t, uint32_t, uint8_t> inherited;
  Rating(): inherited() { }
  Rating(uint32_t a, uint32_t b, uint8_t c): inherited(a,b,c) { }
};
    ...
    while (!feof(f)) {
        ...
        do {
            ...
            if (q == p) 
                break;
            p = q;
            if (k >= NCOLS-1) // skip entry id and seq number
                md[n] = d;
            k++;
        } while (p != NULL);
        n++;
...
public:
  ...
  void sum_dim(D1Array<T> &u, uint32_t dim) const;
...
template<class T> inline int
D2Array<T>::add_to(const T ** const d)
{
  for (uint32_t i = 0; i < _m; ++i)
    for (uint32_t j = 0; j < _n; ++j)
      _data[i][j] += d[i][j];
...
template<class T> inline void
D2Array<T>::sum_dim(D1Array<T> &u, uint32_t dim) const
{
  if (dim == 0) { // sum cols
    assert(_n == u.n());
    T* ud = u.data();
